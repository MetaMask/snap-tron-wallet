...
describe('TransactionMapper', () => {
  ...
  describe('mapTransaction', () => {
    describe('TransferContract (Native TRX transfers)', () => {
      it('should map a native TRX send transaction correctly', () => {
        ...
        console.log('Native TRX Transaction Result:', JSON.stringify(result, null, 2));
        
        expect(result).toBeDefined();
        expect(result).not.toBeNull();
        expect(result!.type).toBe('send');
        expect(result!.id).toBe('8145535b24f71bc592b8ab2d94e91a30d12f74ab33fa4aab2ff2a27b767fc49b');
        expect(result!.from).toHaveLength(1);
        expect(result!.from[0]?.address).toBe('TGJn1wnUYHJbvN88cynZbsAz2EMeZq73yx');
        expect((result!.from[0]?.asset as any)?.amount).toBe('0.01');
        expect((result!.from[0]?.asset as any)?.unit).toBe('TRX');
        expect((result!.from[0]?.asset as any)?.type).toBe('tron:728126428/slip44:195');
        expect(result!.to).toHaveLength(1);
        expect(result!.to[0]?.address).toBe('TEFik7dGm6r5Y1Af9mGwnELuJLa1jXDDUB');
    ...
    describe('TransferAssetContract (TRC10 transfers)', () => {
      it('should map a TRC10 send transaction correctly', () => {
        ...
        console.log('TRC10 Transaction Result:', JSON.stringify(result, null, 2));
        
        expect(result).toBeDefined();
        expect(result).not.toBeNull();
        expect(result!.type).toBe('send');
        expect(result!.id).toBe('d8fc96d5b81fe600e055741e27135e22d5ae42584c9056758f797b1a20328818');
        expect(result!.from).toHaveLength(1);
        expect(result!.from[0]?.address).toBe('TFDP1vFeSYPT6FUznL7zUjhg5X7p2AA8vw');
        expect((result!.from[0]?.asset as any)?.amount).toBe('0.494');
        expect((result!.from[0]?.asset as any)?.unit).toBe('UNKNOWN');
        expect((result!.from[0]?.asset as any)?.type).toBe('tron:728126428/trc10:1002000');
        expect(result!.to).toHaveLength(1);
        expect(result!.chain).toBe('tron:728126428');
        expect(result!.status).toBe('confirmed');
        expect(result!.fees).toHaveLength(1);
    ...
    describe('TriggerSmartContract (TRC20 transfers)', () => {
      it('should map a TRC20 send transaction with assistance data correctly', () => {
        ...
        const trc20AssistanceData = contractInfoMock.data[0] as ContractTransactionInfo;

        const result = TransactionMapper.mapTransaction({
          scope: Network.Mainnet,
          account: mockAccount,
          trongridTransaction: rawTransaction,
          trc20AssistanceData: trc20AssistanceData,
        });

        console.log('TRC20 Transaction Result:', JSON.stringify(result, null, 2));
        
        expect(result).toBeDefined();
        expect(result).not.toBeNull();
        expect(result!.type).toBe('send');
        expect(result!.id).toBe('35f3dcfede12f943827809ddc18b891f78c38337e2b80912f50bd52a054497aa');
        expect(result!.from).toHaveLength(1);
        expect(result!.from[0]?.address).toBe('TGJn1wnUYHJbvN88cynZbsAz2EMeZq73yx');
        expect((result!.from[0]?.asset as any)?.amount).toBe('0.01');
      ...
      it('should return null for TriggerSmartContract without assistance data', () => {
        ...
        console.log('TriggerSmartContract without assistance data Result:', result);
    ...
    describe('Fee calculation', () => {
      ...
      it('should calculate comprehensive fees for TRC20 transfers', () => {
        ...
        console.log('TRC20 comprehensive fees:', JSON.stringify(result?.fees, null, 2));
        expect(result).not.toBeNull();
        expect(result?.fees).toBeDefined();
        expect(result?.fees).toHaveLength(3);
  ...
  describe('mapTransactions', () => {
    it('should map multiple different transaction types correctly', () => {
      ...
      const trc20AssistanceData = contractInfoMock.data as ContractTransactionInfo[];
      ...
      console.log('Mapped multiple transaction types count:', result.filter(tx => tx !== null).length);
      console.log('Sample mapped transactions:', JSON.stringify(result.filter(tx => tx !== null), null, 2));
  ...
  describe('Network-specific behavior', () => {
    it('should work with different networks (Shasta)', () => {
      ...
      console.log('Shasta network transaction result:', JSON.stringify(result, null, 2));
      
      expect(result).toBeDefined();
      expect(result).not.toBeNull();
      expect(result!.chain).toBe('tron:2494104990'); // Shasta chain ID
      expect((result!.from[0]?.asset as any)?.type).toBe('tron:2494104990/slip44:195');
      expect((result!.to[0]?.asset as any)?.type).toBe('tron:2494104990/slip44:195');
    });
  });
});
