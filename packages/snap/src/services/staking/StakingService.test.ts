...
describe('StakingService', () => {
  ...
  describe('buildStakeTransactions', () => {
    it('returns freeze and vote transactions by default', async () => {
      const amount = BigNumber(100);

      const result = await stakingService.buildStakeTransactions({
        account: mockAccount,
    ...
    it('returns only freeze transaction when vote is excluded', async () => {
      const amount = BigNumber(100);
      ...
      expect(mockTronWeb.transactionBuilder.vote).not.toHaveBeenCalled();
    });
  });

  describe('buildUnstakeTransactions', () => {
    it('returns unfreeze transaction', async () => {
      const amount = BigNumber(100);

      const result = await stakingService.buildUnstakeTransactions({
        account: mockAccount,
  ...
  describe('buildClaimUnstakedTransactions', () => {
    it('returns withdrawExpireUnfreeze transaction', async () => {
      const result = await stakingService.buildClaimUnstakedTransactions({
        account: mockAccount,
        scope: Network.Mainnet,
      });
  ...
  describe('buildClaimRewardsTransactions', () => {
    it('returns withdrawBlockRewards transaction', async () => {
      const result = await stakingService.buildClaimRewardsTransactions({
        account: mockAccount,
        scope: Network.Mainnet,
      });
