...
export const AccountRow = ({
  ...
  //     params: {
  //       id: account.id,
  //       assets: [`${network}/${TRON_TOKEN}`],
  //     },
  //   })) as Record<string, Balance>;

  //   setBalance(response?.[`${network}/${TRON_TOKEN}`]?.amount ?? '0');
  // };

  const handleSend = async (id: string) => {
    // Tron transaction flow not yet implemented
    console.log('Tron transaction flow not yet implemented');
  };
  ...
  const USE_CASES = [
    {
      title: 'Tron Transaction (Coming Soon)',
      excludeFromNetworks: [],
      builder: async () => {
